...
import PulserInputHandler from './pulser_input_handler_debranded.js';
import PulserApiConnector from './pulser_api_connector.js';
import pulserExtensions from './pulser-command-extensions.js';
...
function showInteractiveHelp() {
  ...
  console.log('/commands - Show Pulser extension commands (: commands)');
  console.log('\n' + chalk.cyan('Extension Commands:'));
...
function processSlashCommand(command, fullInput, options) {
  ...
  switch (cmd) {
    ...
    case '/local':
      ...
      break;
    case '/commands':
      pulserExtensions.showAvailableCommands();
      break;
    default:
...
async function processCommand(command, options) {
  ...
  // Check if this is a Pulser extension command (starts with :)
  if (pulserExtensions.isPulserCommand(command)) {
    await pulserExtensions.executePulserCommand(command);
    return;
  }
